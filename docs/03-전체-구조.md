...
통합 아키텍처가 양쪽 극단과 비교하여 어떤 위치에 있는지 정량적으로 확인한다.

### 아키텍처별 정량 비교

| 항목 | 실용적 접근 (A) | 엔터프라이즈 접근 (B) | 통합 아키텍처 |
|---|---|---|---|
| 폴더 수 | 5 | 8 | **7** |
| 파일 수 | ~10 | ~25 | **~18** |
| 네스팅 깊이 | 1 | 2~3 | **1** |
...
통합 아키텍처는 **복잡도는 A에 가깝고, 확장성은 B에 가까운** 위치를 목표로 한다. 이를 가능하게 하는 핵심 메커니즘은 **선택적 레이어**와 **평면 우선 구조**의 결합이다.
